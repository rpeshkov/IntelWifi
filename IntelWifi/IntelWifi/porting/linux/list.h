...
static inline bool __list_add_valid(struct list_head *newel,
...
static inline void __list_add(struct list_head *newel,
                              ...
                              struct list_head *next)
{
    if (!__list_add_valid(newel, prev, next))
        return;
    
    next->prev = newel;
    newel->next = next;
    newel->prev = prev;
    WRITE_ONCE(prev->next, newel);
}
 ...
 */
static inline void list_add(struct list_head *newel, struct list_head *head)
{
    __list_add(newel, head, head->next);
}
