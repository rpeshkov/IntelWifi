...
#include "iwlwifi/fw/notif-wait.h"
...
static int iwl_send_calib_cfg(struct iwl_priv *priv)
{
    ...
    calib_cfg_cmd.ucd_calib_cfg.flags = IWL_CALIB_CFG_FLAG_SEND_COMPLETE_NTFY_MSK;
...
static int iwl_send_wimax_coex(struct iwl_priv *priv)
{
    ...
    return iwl_dvm_send_cmd_pdu(priv, COEX_PRIORITY_TABLE_CMD, 0, sizeof(coex_cmd), &coex_cmd);
}

static const u8 iwl_bt_prio_tbl[BT_COEX_PRIO_TBL_EVT_MAX] = {
    ((BT_COEX_PRIO_TBL_PRIO_BYPASS << IWL_BT_COEX_PRIO_TBL_PRIO_POS) | (0 << IWL_BT_COEX_PRIO_TBL_SHARED_ANTENNA_POS)),
    ((BT_COEX_PRIO_TBL_PRIO_BYPASS << IWL_BT_COEX_PRIO_TBL_PRIO_POS) | (1 << IWL_BT_COEX_PRIO_TBL_SHARED_ANTENNA_POS)),
    ((BT_COEX_PRIO_TBL_PRIO_LOW << IWL_BT_COEX_PRIO_TBL_PRIO_POS) | (0 << IWL_BT_COEX_PRIO_TBL_SHARED_ANTENNA_POS)),
    ((BT_COEX_PRIO_TBL_PRIO_LOW << IWL_BT_COEX_PRIO_TBL_PRIO_POS) | (1 << IWL_BT_COEX_PRIO_TBL_SHARED_ANTENNA_POS)),
    ((BT_COEX_PRIO_TBL_PRIO_HIGH << IWL_BT_COEX_PRIO_TBL_PRIO_POS) | (0 << IWL_BT_COEX_PRIO_TBL_SHARED_ANTENNA_POS)),
    ((BT_COEX_PRIO_TBL_PRIO_HIGH << IWL_BT_COEX_PRIO_TBL_PRIO_POS) | (1 << IWL_BT_COEX_PRIO_TBL_SHARED_ANTENNA_POS)),
    ((BT_COEX_PRIO_TBL_PRIO_BYPASS << IWL_BT_COEX_PRIO_TBL_PRIO_POS) | (0 << IWL_BT_COEX_PRIO_TBL_SHARED_ANTENNA_POS)),
    ((BT_COEX_PRIO_TBL_PRIO_COEX_OFF << IWL_BT_COEX_PRIO_TBL_PRIO_POS) | (0 << IWL_BT_COEX_PRIO_TBL_SHARED_ANTENNA_POS)),
    ((BT_COEX_PRIO_TBL_PRIO_COEX_ON << IWL_BT_COEX_PRIO_TBL_PRIO_POS) | (0 << IWL_BT_COEX_PRIO_TBL_SHARED_ANTENNA_POS)),
    0, 0, 0, 0, 0, 0, 0
};
...
void iwl_send_prio_tbl(struct iwl_priv *priv)
{
    ...
    memcpy(prio_tbl_cmd.prio_tbl, iwl_bt_prio_tbl, sizeof(iwl_bt_prio_tbl));
    
    if (iwl_dvm_send_cmd_pdu(priv, REPLY_BT_COEX_PRIO_TABLE, 0, sizeof(prio_tbl_cmd), &prio_tbl_cmd))
...
int iwl_send_bt_env(struct iwl_priv *priv, u8 action, u8 type)
{
    ...
    ret = iwl_dvm_send_cmd_pdu(priv, REPLY_BT_COEX_PROT_ENV, 0, sizeof(env_cmd), &env_cmd);
    if (ret)
...
int IwlDvmOpMode::iwl_alive_notify(struct iwl_priv *priv)
{
    ...
    for (i = 0; i < n_queues; i++)
        if (queue_to_txf[i] != IWL_TX_FIFO_UNUSED)
            iwl_trans_ac_txq_enable(priv->trans, i, queue_to_txf[i], 0);
...
{
    struct iwl_priv *priv = container_of(notif_wait, struct iwl_priv, notif_wait);
    struct iwl_alive_data *alive_data = (struct iwl_alive_data *)data;
    struct iwl_alive_resp *palive;
    
    palive = (struct iwl_alive_resp *)pkt->data;
    
    IWL_DEBUG_FW(priv, "Alive ucode status 0x%08X revision 0x%01X 0x%01X\n",
                 palive->is_valid, palive->ver_type, palive->ver_subtype);
    
    priv->device_pointers.error_event_table = le32_to_cpu(palive->error_event_table_ptr);
    priv->device_pointers.log_event_table = le32_to_cpu(palive->log_event_table_ptr);
...
int IwlDvmOpMode::iwl_load_ucode_wait_alive(struct iwl_priv *priv, enum iwl_ucode_type ucode_type)
{
...
int IwlDvmOpMode::iwl_run_init_ucode(struct iwl_priv *priv)
{
